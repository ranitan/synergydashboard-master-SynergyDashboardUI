...
function showbenchtobillingdata() {
	var filterData = JSON.stringify({
		"IsActive": true
	});

	   callGetListSync('GetResourcesInBenchPlans', filterData, function(e){
	   $("#benchToBillingGrid").dxDataGrid({ dataSource: e })
   })
   renderBenchToBillingReportGrid();
}
...
function renderBenchToBillingReportGrid(){
	var rowNumber = 0;
   var dataGrid = $("#benchToBillingGrid").dxDataGrid({
	   filterRow: {
		   visible: true,
		   applyFilter: "auto"
	   },
	...
	//    }
		   ...
		   cellTemplate: function (container, options) {
			$("<div>")
			   .append("<button class='btn btn-xs btn-primary edit-btn btnSelect'><i class='fas fa-pencil-alt'></i></button><button class='btn btn-xs btn-primary edit-btn' data-rrmid ="+options.value+"  onclick=showskill('"+options.value+"')><i class='fas fa fa-eye'></i></button><button class='btn btn-xs btn-primary edit-btn' onclick=showHistory('"+options.value+"')><i class='fas fa-tasks'></i></button>")
			   .appendTo(container);
			   },
		   onClick: function (e) {
			showskill(data.Id);
