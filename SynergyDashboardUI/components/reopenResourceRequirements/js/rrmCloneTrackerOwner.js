function trackCandidatesRRMOwner(rrmId){    
    getRRMTrackerOwner(rrmId);
    $("#RRMEntryPointTrackerModal").appendTo("body").modal("show");
   
}

function getRRMTrackerOwner(rrmId){  
    setTimeout(function(){
        new SimpleBar(document.getElementById('RRMEntryPointTrackerModalBody'));
    }, 1000);  
    $("#rrmCandidatesTrackerTree").html("");
    ...
    callGetListAsync('GetAllRRMCountByRRMId', filterData, function (data) {
        if(data!=undefined && data.length>0){
            $("#l1InterviewsScheduledRRMCard").html(data[0].L1InterviewScheduled);
            $("#l1InterviewsConductedRRMCard").html(data[0].L1InterviewConducted);
            $("#l2InterviewsScheduledRRMCard").html(data[0].L2InterviewScheduled);
            $("#l2InterviewsConductedRRMCard").html(data[0].L2InterviewConducted);                
            $("#offerMadeRRMCard").html(data[0].OfferMade);
            $("#offerAcceptedRRMCard").html(data[0].offerAccepted);
        }
    });
    callGetListAsync('GetCandidatesForRRMTracker', filterData, function (e) {
        if(e.length == 0){
            $("#rrmCandidatesTrackerTree").html("<h3>No Data Available</h3>");
        }
        var trackerhtml = "";
        ...
        e.forEach(function (key, item) {
            ...
            trackerhtml += "</ul>"
            trackerhtml +="</div>"
            $("#rrmCandidatesTrackerTree").html(trackerhtml);        
        });        
    });
}

function closeRRMTracker(){
    $("#RRMEntryPointTrackerModal").modal("hide");
}
...
$(document).on("click", ".rrm-candidate-tracker", function (e) {
    ...
    $("#RRMCandidateTrackerModal").appendTo("body").modal("show");
    ...
    setTimeout(function(){
        new SimpleBar(document.getElementById('RRMPCandidatetrackerModalBody'));
    }, 1000);   
});

function openCandidateTrackerOwner(interviewStages){    
    var datagrid = $("#sdgd-rrmCandidateTracker").dxDataGrid({
...
$(document).on("click", "#rrm_entryPointRemarksId", function (e) {
    ...
    $('#rrm_rrmEntryPointClone_remarks').appendTo("body").modal("show");
    $("#remarksBodyRRMCloneEntryPoint").empty();
    $("#remarksBodyRRMCloneEntryPoint").html(remark);
  })
  
  function closeRRMCloneEntryPoint(){
    $('#rrm_rrmEntryPointClone_remarks').modal("hide");
    // $("#remarksBodyRRMCloneEntryPoint").empty();
  }

  function closeOpenRRMCloneEntry(){
    $('#rrm_OpenClosedRRM').modal("hide");
    $("#OpenRRMEnterBody").empty();
  }
...
function mapCandidateTimeLineRRM(interviewStages,profileRRMDetails){
    ...
    timeLineHtml +="</div>"
    $("#candidateTimeLineRRM").html(timeLineHtml)
}

function closeRRMCandidateTracker(){
    $("#RRMCandidateTrackerModal").modal('hide');
}