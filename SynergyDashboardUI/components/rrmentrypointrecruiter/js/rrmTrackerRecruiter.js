function trackCandidatesRRMRecruiter(rrmId){    
    getRRMTrackerRecruiter(rrmId);
    $("#RRMEntryPointRecruiterTrackerModal").appendTo("body").modal("show");

    setTimeout(function(){
        new SimpleBar(document.getElementById('RRMEntryPointRecruiterRecruiterModalBody'));
    }, 1000);
}

function getRRMTrackerRecruiter(rrmId){    
    $("#rrmRecruiterCandidatesTrackerTree").html("");
    ...
    callGetListAsync('GetCardDetailsForTracker',filterData,function(data){
        if(data!=undefined && data.length>0){
            $("#cardCandidateRRMRecruiter").html(data[0].Candidates);
            $("#cardOfferedRRMRecruiter").html(data[0].Offered);
            $("#cardPendingRRMRecruiter").html((data[0].Candidates-data[0].Offered));
            $("#cardOnBoardRRMRecruiter").html(data[0].OnBoard);
        }
    });
    callGetListAsync('GetAllRRMCountByRRMId', filterData, function (data) {
        if(data!=undefined && data.length>0){
            $("#l1InterviewsScheduledRRMRecruiterCard").html(data[0].L1InterviewScheduled);
            $("#l1InterviewsConductedRRMRecruiterCard").html(data[0].L1InterviewConducted);
            $("#l2InterviewsScheduledRRMRecruiterCard").html(data[0].L2InterviewScheduled);
            $("#l2InterviewsConductedRRMRecruiterCard").html(data[0].L2InterviewConducted);                
            $("#offerMadeRRMRecruiterCard").html(data[0].OfferMade);
            $("#offerAcceptedRRMRecruiterCard").html(data[0].offerAccepted);
        }
    });
    callGetListAsync('GetCandidatesForRRMTracker', filterData, function (e) {
        if(e.length == 0){
            $("#rrmRecruiterCandidatesTrackerTree").html("<h3>No Data Available</h3>");
        }
        var trackerhtml = "";
        ...
        e.forEach(function (key, item) {
            ...
            trackerhtml += "</ul>"
            trackerhtml +="</div>"
            $("#rrmRecruiterCandidatesTrackerTree").html(trackerhtml);        
        });        
    });
}

function closeRRMTrackerRecruiter(){
    $("#RRMEntryPointRecruiterTrackerModal").modal("hide");
}

$(document).on("click", ".rrmrecruiter-candidate-tracker", function (e) {
    ...
    $("#RRMRecruiterCandidateTrackerModal").appendTo("body").modal("show");
    openCandidateTrackerRecruiter(interviewStages);
    var profileRRMDetails = callgetlist("GetProfileAndRRMDetailsForTracker",filterData1);
    mapCandidateTimeLineRRMRecruiter(interviewStagestimeline,profileRRMDetails);
    setTimeout(function(){
        new SimpleBar(document.getElementById('RRMRecruiterCandidatetrackerModalBody'));
    }, 1000);
});

function openCandidateTrackerRecruiter(interviewStages){    
    var datagrid = $("#sdgd-rrmRecruiterCandidateTracker").dxDataGrid({
        ...
        columns: [
          {
            ...
            cellTemplate: function (container, options) {
              ...
              .append("<img id='imgCandidateTrackerRRMRecruiter-"+options.data.Id+"' src='"+options.data.ProfilePicture+"' class='img-circle interviewer-image' width='25px' height='25px'></img><div id='imageTooltipRRMRecruiterCandidateTracker"+options.data.Id+"'></div>")
              .appendTo(container);
              prepareImageTooltipForCandidateRRMRecruiterTracker(options.data.Id,options.data.ProfilePicture,options.data.InterviewerName)
            }
          },
          {
            dataField: "InterviewerName",
            caption: "Interviewer/Recruiter"
          },
          {
            dataField: "Status",
            caption: "Status",
            ...
            cellTemplate: function (container, options) {
                if(options.data.OfferStatus == ""){
                    if(options.data.Comments != null && options.data.Comments.replace(" ","") != ""){
                        $("<div>")
                        .append("<a id='rrm_recruiter_remarksId' rrm_remarks='"+options.data.Comments+"' style='cursor:pointer;text-decoration:underline;'>Remarks</a>")
                        .appendTo(container);
                    }     
                    else{
                        $("<div>")
...
function generatePopoverForRRMRecruiterCandidatetrackerRemark(data){
    $("#remarksTooltipRRMRecruiterCandidateTracker_"+data.Id).dxTooltip({
        target: "#remarksForRRMRecruiterCandidateTracker_"+data.Id,
...
$(document).on("click", "#rrm_recruiter_remarksId", function (e) {
    var remark =$(this).attr('rrm_remarks');
    $('#rrm_recruiter_remarks').appendTo("body").modal("show");
    $("#remarksBodyRRMRecruiter").empty();
    $("#remarksBodyRRMRecruiter").html(remark);
  })
  
  function closeRRMRecruiter(){
    $('#rrm_recruiter_remarks').modal("hide");
    $("#remarksBodyRRMRecruiter").empty();
  }

  
function prepareImageTooltipForCandidateRRMRecruiterTracker(Id,ProfilePicture,InterviewerName){
    $("#imageTooltipRRMRecruiterCandidateTracker"+Id).dxTooltip({
      target: "#imgCandidateTrackerRRMRecruiter-"+Id,
      showEvent: "mouseenter",
      hideEvent: "mouseleave",
      closeOnOutsideClick: false,
      position: "right",
...
function mapCandidateTimeLineRRMRecruiter(interviewStages,profileRRMDetails){
    ...
    timeLineHtml +="</div>"
    $("#candidateTimeLineRRMRecruiter").html(timeLineHtml)
}

function closeRRMRecruiterCandidateTracker(){
    $("#RRMRecruiterCandidateTrackerModal").modal("hide");
}