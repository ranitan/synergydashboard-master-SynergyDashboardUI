...
const getCaseStudyData = () => {
    ...
    //var getCaseStudyList = callgetlist('getCaseStudy', filterData);
    getCaseStudyList = [
        {
          "Id": "8FA96F91-D8CF-4AE2-BC98-C1FE25ECDFD$",
          "CaseNo": "CS0001",
          "Title": "Wordpress Casestudy",
          "Technology": "PHP, Wordpress & Mysql",
          "Description": "Wordpress task, Ecommmerce website for a retailer.",
          "IsActive": true,
          "CreatedBy": "0439 - Leejoy",
          "CreatedById": "9EC36C10-2D99-47C9-B610-2E4DFF8C6553",
          "ModifiedBy": "0439 - Leejoy",
          "ModifiedById": "9EC36C10-2D99-47C9-B610-2E4DFF8C6553"
        },
        {
          "Id": "1A35B6F3-9E40-47D6-B32D-A99B9BD447BE",
          "CaseNo": "CS0002",
          "Title": "Student Portal Casestudy",
          "Technology": "PHP & Mysql",
          "Description": "Student Portal website for a customer.",
          "IsActive": true,
          "CreatedBy": "0439 - Leejoy",
          "CreatedById": "9EC36C10-2D99-47C9-B610-2E4DFF8C6553",
          "ModifiedBy": "0439 - Leejoy",
      ...
      getCaseStudyList.map(function (res) {
        return res.Description = removeTags(res.Instruction)
    })
    ...
    // })
    renderCaseStudyListGrid(getCaseStudyList);
}
getCaseStudyData();
...
function DownloadCaseStudy(mode, SARoutineId = null) {
    ...
    if (mode == 'edit') {
        ...
        var getCaseStudyData = getSARoutineById[0];
        var editEndDate = (getCaseStudyData.EndDate == "1900-01-01T00:00:00" || getCaseStudyData.EndDate == null) ? $("#routine_end_date").val('') : $("#routine_end_date").val(moment(getCaseStudyData.EndDate).format('MM/DD/YYYY'));
        $("#task").val(getCaseStudyData.Task);
        $("#routine_start_date").val(moment(getCaseStudyData.StartDate).format('MM/DD/YYYY'));

        $(`input[name='freqoption'][value=${getCaseStudyData.Frequency}]`).prop('checked', true);
        $("#routine_time").val(getCaseStudyData.TimeOfExecution);
        $("#masterinstructions").dxHtmlEditor("instance").option('value', getCaseStudyData.Instruction);
        $('#saroutineid').val(getCaseStudyData.Id)
...
function CommentCaseStudy(mode, SARoutineId = null) {
...
function renderCaseStudyListGrid(data) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    cellTemplate: function (container, options) {
                        ...
                        domActions +=
                            `<button class='btn btn-xs btn-primary edit-btn sa-edit' onclick=DownloadCaseStudy('view','${options.data.Id}')><i class='fas fa-download'></i></button>`;
                        domActions +=
                            `<button class='btn btn-xs btn-primary edit-btn sa-edit' style="display:block; margin-left:5px;" onclick=CommentCaseStudy('report','${options.data.Id}')><i class="fas fa-comment"></i></button>`;
                        domActions +=
                            `<button class='btn btn-xs btn-primary edit-btn sa-edit' style="display:block; margin-left:5px;" onclick=EditCaseStudy('history','${options.data.Id}')><i class="fas fa-pencil-alt"></i></button>`;

                        if (getCurrentUserId == getTaskCreatedUserId) {
                            domActions +=
                                `<button class='btn btn-xs btn-primary edit-btn sa-edit' style="display:block; margin-left:5px;" onclick=DeleteCaseStudy('edit','${options.data.Id}')><i class='fas fa-trash'></i></button>`;
                            $("<div class='text-center' style='display:flex;'>")
                                .append($(domActions))
                                .appendTo(container);
                        } else {
                            domActions +=
                                `<button class='btn btn-xs btn-primary edit-btn sa-edit' style="display:none;" onclick=DownloadCaseStudy('edit','${options.data.Id}')><i class='fas fa-pencil-alt'></i></button>`;
                            $("<div class='text-center' style='display:flex;'>")
                                .append($(domActions))
                                .appendTo(container);
                        }
