...
function rab_showbenchtobillingdata() {
	var filterData = JSON.stringify({
		"IsActive": true
	});

	   callGetListSync('GetResourcesinAverageBillingPlans', filterData, function(e){
	   $("#ResourcesinAverageBillingGrid").dxDataGrid({ dataSource: e })
   })
}

//convert date format 
function rab_convertDate(BenchStartDate){
...
function renderResourcesinAverageBenchToBillingReportGrid(){
	var rowNumber = 0;
   var dataGrid = $("#ResourcesinAverageBillingGrid").dxDataGrid({
	   filterRow: {
		   visible: true,
		   applyFilter: "auto"
	   },
	...
	//    }
		   ...
		   cellTemplate: function (container, options) {
			$("<div>")
			   .append("<button class='btn btn-xs btn-primary edit-btn btnSelect'><i class='fas fa-pencil-alt'></i></button><button class='btn btn-xs btn-primary edit-btn' data-rrmid ="+options.value+"  onclick=showskill('"+options.value+"')><i class='fas fa fa-eye'></i></button>")
			   .appendTo(container);
			   },
		   onClick: function (e) {
			showskill(data.Id);
