function trackCandidatesRRMOwner(closedrrmId){    
    getRRMTrackerOwner(closedrrmId);
    $("#closedRRMEntryPointTrackerModal").appendTo("body").modal("show");
   
}

function getRRMTrackerOwner(closedrrmId){  
    setTimeout(function(){
        new SimpleBar(document.getElementById('closedRRMEntryPointTrackerModalBody'));
    }, 1000);  
    $("#closedrrmCandidatesTrackerTree").html("");
    ...
    callGetListAsync('GetCardDetailsForTracker',filterData,function(data){
        if(data!=undefined && data.length>0){
            $("#cardCandidateRRM").html(data[0].Candidates);
            $("#cardOfferedRRM").html(data[0].Offered);
            $("#cardPendingRRM").html((data[0].Candidates-data[0].Offered));
            $("#cardOnBoardRRM").html(data[0].OnBoard);
        }
    });
    callGetListAsync('GetAllRRMCountByRRMId', filterData, function (data) {
        if(data!=undefined && data.length>0){
            $("#closedl1InterviewsScheduledRRMCard").html(data[0].L1InterviewScheduled);
            $("#closedl1InterviewsConductedRRMCard").html(data[0].L1InterviewConducted);
            $("#closedl2InterviewsScheduledRRMCard").html(data[0].L2InterviewScheduled);
            $("#closedl2InterviewsConductedRRMCard").html(data[0].L2InterviewConducted);                
            $("#closedofferMadeRRMCard").html(data[0].OfferMade);
            $("#closedofferAcceptedRRMCard").html(data[0].offerAccepted);
        }
    });
    callGetListAsync('GetCandidatesForRRMTracker', filterData, function (e) {
        if(e.length == 0){
            $("#closedrrmCandidatesTrackerTree").html("<h3>No Data Available</h3>");
        }
        var trackerhtml = "";
        var length = ((e.length*10)+35)+"em";
        $("#cardDetailsTrackerRRM").attr("style","width:"+length)
        e.forEach(function (key, item) {
            ...
            trackerhtml += "</ul>"
            trackerhtml +="</div>"
            $("#closedrrmCandidatesTrackerTree").html(trackerhtml);        
        });        
    });
}

function closedcloseRRMTracker(){
    $("#closedRRMEntryPointTrackerModal").modal("hide");
}

$(document).on("click", ".rrm-candidate-tracker", function (e) {
    var closedrrmId = $(e.currentTarget).data("rrmid");
    ...
    $("#closedRRMCandidateTrackerModal").appendTo("body").modal("show");
    openCandidateTrackerOwner(interviewStages);
    var profileRRMDetails = callgetlist("GetProfileAndRRMDetailsForTracker",filterData1);
    mapCandidateTimeLineRRM(interviewStagestimeline,profileRRMDetails);
    setTimeout(function(){
        new SimpleBar(document.getElementById('closedRRMPCandidatetrackerModalBody'));
    }, 1000);   
});

function openCandidateTrackerOwner(interviewStages){    
    var datagrid = $("#closedsdgd-rrmCandidateTracker").dxDataGrid({
        ...
        columns: [
          {
            ...
            cellTemplate: function (container, options) {
              ...
              .append("<img id='imgCandidateTrackerRRM-"+options.data.Id+"' src='"+options.data.ProfilePicture+"' class='img-circle interviewer-image' width='25px' height='25px'></img><div id='imageTooltipRRMCandidateTracker"+options.data.Id+"'></div>")
              .appendTo(container);
              prepareImageTooltipForCandidateRRMTracker(options.data.Id,options.data.ProfilePicture,options.data.InterviewerName)
            }
          },
          {
            ...
            cellTemplate: function (container, options) {
                if(options.data.OfferStatus == ""){
                    if(options.data.Comments != null && options.data.Comments.replace(" ","") != ""){
                        $("<div>")
                        .append("<a id='rrm_entryPointRemarksId' rrm_remarks='"+options.data.Comments+"' style='cursor:pointer;text-decoration:underline;'>Remarks</a>")
                        .appendTo(container);
                    }     
                    else{
                        $("<div>")
...
function generatePopoverForRRMCandidatetrackerRemark(data){
    $("#remarksTooltipRRMCandidateTracker_"+data.Id).dxTooltip({
        target: "#remarksForRRMCandidateTracker_"+data.Id,
...
$(document).on("click", "#rrm_entryPointRemarksId", function (e) {
    var remark =$(this).attr('rrm_remarks');
    $('#closedrrm_rrmEntryPoint_remarks').appendTo("body").modal("show");
    $("#closedremarksBodyRRMEntryPoint").empty();
    $("#closedremarksBodyRRMEntryPoint").html(remark);
  })
  
  function closedcloseRRMEntryPoint(){
    $('#closedrrm_rrmEntryPoint_remarks').modal("hide");
    $("#closedremarksBodyRRMEntryPoint").empty();
  }
  
function prepareImageTooltipForCandidateRRMTracker(Id,ProfilePicture,InterviewerName){
    $("#imageTooltipRRMCandidateTracker"+Id).dxTooltip({
      target: "#imgCandidateTrackerRRM-"+Id,
      showEvent: "mouseenter",
      hideEvent: "mouseleave",
      closeOnOutsideClick: false,
      position: "right",
...
function mapCandidateTimeLineRRM(interviewStages,profileRRMDetails){
    ...
    timeLineHtml +="</div>"
    $("#closedcandidateTimeLineRRM").html(timeLineHtml)
}

function closedcloseRRMCandidateTracker(){
    $("#closedRRMCandidateTrackerModal").modal('hide');
}