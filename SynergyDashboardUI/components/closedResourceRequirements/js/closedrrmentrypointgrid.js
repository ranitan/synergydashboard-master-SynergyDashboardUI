closedRRMEntryPointGridOwner= (function (pageId) {
    var _rrmGrid = {};
    _rrmGrid.ui = {};
...
function exportSingleRRMEntry(data,e){
    ...
    var closedinterviewerResult = callgetlist('GetMappedInterviewersForRRM', filter_val1);
    console.log(closedinterviewerResult,"Hello")
    $(closedinterviewerResult).each(function(idx,obj){
    ...
    getRRMSkillsForExport(closedrrmId)
    var skillGrid = $("#closedskillForRRMExport").dxDataGrid("instance");
    ...
    setTimeout(() => {
        DevExpress.excelExporter.exportDataGrid({
            ...
            workbook.xlsx.writeBuffer().then(function (buffer) {
                ...
                $("#closedskillForRRMExport").dxDataGrid("instance").dispose();
            });
        });
    }, 500);
...
function getRRMSkillsForExport(closedrrmId){
    var filter_val = JSON.stringify({
        "ResourceRequirementId ": closedrrmId,
        "IsActive": "True",
        "Token": Token
    ...
    $("#closedskillForRRMExport").dxDataGrid({
...
function closedGetRRMCommentsEntryPoint(RRMId,RRMNo){
    
    if(RRMNo != null && RRMNo !="" && RRMNo !=undefined){
        $("#RRMEntryPointCommentsRRMNo").html("- " +RRMNo);
    }   
    $('#closedrrm_common_comments').appendTo("body").modal("show");
    getManageCommonCommentHistory(RRMId);
    var editor = CKEDITOR.instances.closedsd_txtEditor_RRMComments_common;
        if (editor) {
            editor.destroy(true);
        }
        $("#closedsd_txtEditor_RRMComments_common").dxHtmlEditor('instance').option('value',"");

}

function closedbindCommonForRRMComments(){
    
    var commentsEditor = $("#closedsd_txtEditor_RRMComments_common").dxHtmlEditor({
        height: 300,
        toolbar: {
            items: [
                "undo", "redo", "separator",
    ...
    var mentionToolTipRRM = $("#closedsd_popover_RRMmentions_common").dxPopover({
...
$(document).on("mouseover", "#closedrrm_common_comments .dx-mention", function (e) {
...
function LoadTooltipForMentionRRMCommon(Id,MentionType,Target){
    if(MentionType == "@"){  
        ...
        callGetListAsync('GetEmployeeDetailsForMentions', filterData, function(e){
            var Details = e[0];
            $("#closedsd_popover_RRMmentions_common").dxPopover('instance').option('target',Target);  
            $("#closedsd_popover_RRMmentions_common").dxPopover('instance').option("contentTemplate",function(data) {
 ...
 function getManageCommonCommentHistory(ResourceRequirementId) {
    ...
    callGetListAsync('GetCommentsInResourceRequirement', filter_val, function (getManageplanComments) {
        ...
        $("#closedrrm_common_manage_plan_comments_documents").show();
        $("#closedrrm_common_manage_plan_comments_documents").html(plan_comment_history_html);
        var div = document.getElementById('closedrrm_common_manage_plan_comments_documents');
        div.scrollTop = div.scrollHeight - div.clientHeight;
    })
}

function closedsaveCommonComments(){
    var finalComments = $("#closedsd_txtEditor_RRMComments_common").dxHtmlEditor('instance').option('value');
    ...
    if (finalComments == "" || finalComments == null || finalComments == undefined) {
        ...
        PostDataCallAsync(dataComment, function (resultComments) {
            ...
            $("#closedrrm_common_manage_plan_comments_documents").empty();
...
function closedcloseRRMCommon(){
    $('#closedrrm_common_comments').modal("hide");
    $("#closedsd_txtEditor_RRMComments_common").dxHtmlEditor('instance').option('value',"");
    RRMIdCommon = null;
    var editor = CKEDITOR.instances.closedsd_txtEditor_RRMComments_common;
        if (editor) {
        ...
        CKEDITOR.replace('closedsd_txtEditor_RRMComments_common');
}
