...
$(document).on('click','.close',function(){
  timesheetreportedit_getReportData();
});

function OpenTimesheetEdit(id,proname,reportFile){
  ts_projectId = id;
  var selectMonth= getMonth($("#timesheetEditMonth").dxSelectBox('instance').option('value'));
  var selectYear= $("#timesheetEditYear").dxSelectBox('instance').option('value');
  ...
  var endDate = new Date(selectYear,calenderMonth + 1, 0);
   ...
   toDate=getFormattedDate(endDate);
    ...
    $('#TimesheetReportModalEdit').modal('show');
    $('#projectNameHeaderEdit').text(proname);
        var filter_file_val = JSON.stringify({
        "Token":"",
        "Id":id,
        "Month":selectMonth,
        "Year":$("#timesheetEditYear").dxSelectBox('instance').option('value')
     });
...
function getSummarizedTimeSheetListEdit(filterdata) {
  ...
  function renderSummarizedTimeSheeGridEdit(data){
    ...
    $('#divSummarizedTimesheetEdit').dxDataGrid({
        dataSource: data,
        keyExpr: 'SNo',
        showBorders: true,
        sorting :false,
  ...
  function getDetailedTimeSheetListEdit(filterval) {
    ...
    renderDetailedTimesheetEditGrid(GetTimeSheetDetails)
  }


function renderDetailedTimesheetEditGrid(data){
...
/*const employees = [{
 ...
 SNo: 3,
  ...
  TotalHours: '16',
    ...
    $('#divDetailedTimesheetEdit').dxDataGrid({
        dataSource: data,
        keyExpr: 'SNo',
        sorting :false,
        filterRow: {
  ...
  const logSheetCaptionedit = 'Log Sheet Timesheet';
  const summarizedCaptionedit = 'Summarized Timesheet';
  const detailedCaptionedit = 'Detailed Timesheet';
  var borderStylePattern = { style: 'thin', color: { argb: 'FF7E7E7E' } };
  var specialRowIndexes = [];

  function updateTimeSheet(e)
  {
     var tsselectMonth= getMonth($("#timesheetEditMonth").dxSelectBox('instance').option('value'));
     var tsselectYear= $("#timesheetEditYear").dxSelectBox('instance').option('value');
  ...
  function exportLogSheet(ownername, exportFile){
    $('#exportButton').dxButton({
      ...
      onClick() {
        const dataGrid1 = $('#divSummarizedTimesheetEdit').dxDataGrid('instance');
        const dataGrid2 = $('#divDetailedTimesheetEdit').dxDataGrid('instance');
